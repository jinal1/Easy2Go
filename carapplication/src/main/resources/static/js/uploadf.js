...
App.controller('MyCtrl', ['$scope', '$http', '$timeout', '$compile', 'Upload','$state',function ($scope, $http, $timeout, $compile, Upload,$state) {
...
$scope.pFile;
  ...
  /*  $scope.errorMsg = null;
    if ($scope.howToSend === 1) {
	  ...
	  $http({
		  method: 'POST',
		  url: 'http://localhost:8080/saveimage',
		  data: formData,
		  headers: {'Content-Type': undefined }
		}).then(function successCallback(response) {
		    // this callback will be called asynchronously
		    // when the response is available
			 $scope.successMessage = "Owner Detail Submitted Succesfully and give account details";
